...
-- Interactions table — standalone rows (no session join needed) for analytics.
...
create index if not exists interactions_action_type_idx
  on public.interactions (action_type);

alter table public.interactions disable row level security;

drop policy if exists interactions_service_role_access on public.interactions;
create policy interactions_service_role_access
...
drop policy if exists interactions_anon_access on public.interactions;
create policy interactions_anon_access
  on public.interactions
  for all
  to anon
  using (true)
...
grant usage, select on sequence public.interactions_id_seq to anon;
